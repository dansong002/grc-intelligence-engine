...
2. **The engine classifies** the initiative into one or more system archetypes using keyword signal matching (no ML). The classification is editable — remove a wrong archetype or add a missing one from the full set, and the package recomputes instantly. An optional **industry profile** (Retail & Fuel, Banking, Crypto, Manufacturing & Defense, Healthcare, or General) activates the vertical's elevated risks from the same curated library — which in turn bring their controls, evidence, and frameworks into scope — and weights the business-impact analysis toward what matters most to the sector. Industry-activated risks are tagged in the register so it stays clear why each is in scope. The profile persists across runs.
